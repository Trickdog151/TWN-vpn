...
class ConfigLoader(object):
    ...
    def parse_global_section(self, config):
        ...
        for var in global_vars:
            try:
                self.settings[var] = config.get('Nixtrix', var)
...
class OpenvpnHtmlPrinter(object):
    ...
    def print_html_header(self):
        ...
        output('<title>{0!s} Connection Monitor</title>'.format(self.site))
        output('<meta http-equiv="refresh" content="120" />')
        ...
        output('<link rel="stylesheet" href="css/bootstrap-theme.min.css" />')
        if self.maps:
            ...
            output('<link rel="stylesheet" href="leaflet.css" />') 
        # js
...
#        output('});</script>')
        if self.maps:
            ...
            output('<script src="leaflet.js"></script>')
        output('</head><body>')
        ...
        output('<a class="navbar-left" href="#">')
        output('<img src="nixtrix.png"</a>')
    ...
    def print_html_footer(self):
        ...
        output('Page automatically reloads every 2 minutes.')
...
def get_args():
    ...
    parser.add_argument('-c', '--config', type=str,
                        required=False, default='./nixtrix.conf',
                        help='Path to config file nixtrix.conf')
